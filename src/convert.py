...
def convert_and_upload_supervisely_project(
    ...
    def create_ann(image_path):
        ...
        if annotations is not None:
            ...
            for curr_data in ann_data:
                if len(curr_data) != 0:
                    obj_class = idx_to_obj_class[curr_data[0]]
    ...
    for curr_ann_data in all_ann_data:
        ...
        for category in curr_json_data["categories"]:
            if category["id"] not in list(idx_to_obj_class.keys()):
                obj_class = sly.ObjClass(category["name"], sly.AnyGeometry)
                # obj_class_rect = sly.ObjClass(category["name"] + bbox_suffix, sly.Rectangle)
                idx_to_obj_class[category["id"]] = obj_class
